    ...
    public static TermuxSession execute(@NonNull final Context currentPackageContext, @NonNull ExecutionCommand executionCommand,
        ...
        if (executionCommand.executable == null) {
            if (!executionCommand.isFailsafe) {
                ...
                File tmuxBinary = new File(defaultBinPath, "tmux");
                if (preferences != null && preferences.isAutoStartTmuxEnabled() && tmuxBinary.exists() && tmuxBinary.canExecute()) {
                    executionCommand.executable = defaultBinPath + "/sh";
                    executionCommand.arguments = new String[]{
                        "-c",
                        "exec " + tmuxBinary.getAbsolutePath() + " attach-session -t main 2>/dev/null || exec " + tmuxBinary.getAbsolutePath() + " new-session -s main"
                    };
                } else {
